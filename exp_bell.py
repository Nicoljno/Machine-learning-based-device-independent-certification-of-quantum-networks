...
def custom_loss(train, y_pred):
    filename = 'matrici_mes_constraints_bell_lev_1'
    with open(filename, 'rb') as f:
        mes_diz = pickle.load(f)
    filename = 'matrici_non_mes_constraints_bell_lev_1'
    ...
    errors=np.array([0.018709988592, 0.018709988592, 0.018709988592, 0.018709988592,
                     ...
                     0.018709988592, 0.018709988592, 0.018709988592, 0.018709988592,
                     0.018709988592, 0.018709988592, 0.018709988592, 0.018709988592])
    errors=errors*sqrt(-log(10**(-6)*0.5))/sqrt(-log(10**(-1)*0.5))
    ...
    '''
    #errori hoeffding da dati sperimentali
    ...
    for _ in range(1):
        ...
        gamma=(sum(mes_diz[i]*expected_value(p, i, parties, inputs) for i in mes_diz.keys() )+
               sum(non_mes_diz[keys[i]]*(y_pred[_][-(16+i)]) for i in range(len(keys)) )
               )
...
visible = Input(shape=(16,))
hidden1 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(visible)
hidden2 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden1)
hidden3 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden2)
hidden4 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden3)
hidden5 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden4)
hidden6 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden5)
hidden7 = Dense(80, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden6)
output1 = Dense(2, activation = 'tanh', activity_regularizer=regularizers.l2(1e-6))(hidden7)
output2 = Dense(16, activation = 'tanh', activity_regularizer=regularizers.l2(1e-6))(hidden7)
...
model.save('hoeff_bell_exp_5', overwrite=True)
