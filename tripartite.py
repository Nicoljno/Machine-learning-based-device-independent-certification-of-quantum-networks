...
def custom_loss(train, y_pred):
    ...
    for _ in range(1):
        ...
        #I.append(y_pred[_][56]+y_pred[_][57]+y_pred[_][58]+y_pred[_][60]-y_pred[_][59]-y_pred[_][61]-y_pred[_][62]-y_pred[_][63])
        #I.append(-sum((-1)**(a+b)*y_pred[_][32+16+a*4+b*2+c] for a in vals for b in vals for c in vals)-
        #         sum((-1)**(a+c)*y_pred[_][32+8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
        #         sum((-1)**(a+b)*y_pred[_][16+a*4+b*2+c] for a in vals for b in vals for c in vals)+
        #         sum((-1)**(a+c)*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
        #         2*(sum((-1)**(a+b+c)*y_pred[_][a*4+b*2+c] for a in vals for b in vals for c in vals))+
        #         2*(sum((-1)**(a+b+c)*y_pred[_][32+a*4+b*2+c] for a in vals for b in vals for c in vals)))
        I.append(sum((-1)**(a+b+c)*y_pred[_][a*4+b*2+c] for a in vals for b in vals for c in vals)+
                 sum((-1)**(a+b+c)*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
                 sum((-1)**(a+b+c)*y_pred[_][16+a*4+b*2+c] for a in vals for b in vals for c in vals)-
        ...
        Gamma=(Id+A0A1*(y_pred[_][64]+train[_][0])+B0B1*(y_pred[_][65]+train[_][1])+C0C1*(y_pred[_][66]+train[_][2])+
               ...
               C1*sum((-1)**c*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A0B0*sum((-1)**(a+b)*y_pred[_][a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A0B1*sum((-1)**(a+b)*y_pred[_][16+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A0C0*sum((-1)**(a+c)*y_pred[_][a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A0C1*sum((-1)**(a+c)*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A1B0*sum((-1)**(a+b)*y_pred[_][32+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A1B1*sum((-1)**(a+b)*y_pred[_][32+16+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A1C0*sum((-1)**(a+c)*y_pred[_][32+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A1C1*sum((-1)**(a+c)*y_pred[_][32+8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               B0C0*sum((-1)**(b+c)*y_pred[_][a*4+b*2+c] for a in vals for b in vals for c in vals)+
               B0C1*sum((-1)**(b+c)*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               B1C0*sum((-1)**(b+c)*y_pred[_][16+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               ...
               A0B0C1*sum((-1)**(a+b+c)*y_pred[_][8+a*4+b*2+c] for a in vals for b in vals for c in vals)+
               A0B1C0*sum((-1)**(a+b+c)*y_pred[_][16+a*4+b*2+c] for a in vals for b in vals for c in vals)+
        ...
        '''
               A0B0C0*(y_pred[_][56]+train[_][63])+
               A0B0C1*(y_pred[_][57]+train[_][64])+
               A0B1C0*(y_pred[_][58]+train[_][65])+
               A0B1C1*(y_pred[_][59]+train[_][66])+
               A1B0C0*(y_pred[_][60]+train[_][67])+
               A1B0C1*(y_pred[_][61]+train[_][68])+
               A1B1C0*(y_pred[_][62]+train[_][69])+
               A1B1C1*(y_pred[_][63]+train[_][70]))
        '''
...
hidden1 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(visible)
hidden2 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden1)
hidden3 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden2)
hidden4 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden3)
hidden5 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden4)
hidden6 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden5)
hidden7 = Dense(500, activation = 'elu', activity_regularizer=regularizers.l2(1e-6))(hidden6)
...
output9 = Dense(63, activation = 'tanh', activity_regularizer=regularizers.l2(1e-6))(hidden7)
